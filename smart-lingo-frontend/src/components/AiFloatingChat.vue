<template>
  ...
  >
    ...
    <transition name="scale-fade">
      <div v-if="isOpen" ref="chatWindowRef" class="mb-4 w-96 bg-white/80 dark:bg-gray-800/90 backdrop-blur-xl border border-white/20 dark:border-gray-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-colors duration-200" style="height: 500px;">
        <!-- Header -->
        <div 
          class="bg-indigo-600/90 dark:bg-indigo-900/90 p-4 text-white flex justify-between items-center backdrop-blur-sm cursor-pointer select-none"
          @mousedown="startDrag"
        >
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-transparent" ref="messagesContainer">
          <div v-for="(msg, index) in messages" :key="index" :class="['flex', msg.role === 'user' ? 'justify-end' : 'justify-start']">
            <div 
              :class="['max-w-[85%] rounded-2xl p-3 text-sm shadow-sm transition-colors duration-200', 
                msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-bl-none']"
            >
              <div v-html="renderMarkdown(msg.text)" class="prose prose-sm prose-invert dark:prose-invert"></div>
            </div>
          </div>
          <div v-if="isLoading" class="flex justify-start">
             <div class="bg-white dark:bg-gray-700 rounded-2xl rounded-bl-none p-3 shadow-sm transition-colors duration-200">
        ...
        <!-- Input -->
        <div class="p-3 bg-white/50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 backdrop-blur-sm transition-colors duration-200">
          <div class="flex items-center space-x-2">
             <input 
               v-model="inputText" 
               ...
               type="text" 
               placeholder="请输入问题..." 
               class="flex-1 bg-white/80 dark:bg-gray-700/80 border-none rounded-full px-4 py-2 text-sm text-gray-800 dark:text-gray-100 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm transition-colors duration-200"
             />
             <button 
               @click="sendMessage" 
               :disabled="!inputText"
