  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50">
    <div class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
      <div class="text-center mb-8">
        ...
        <p class="text-gray-500 mt-2">登录您的 SmartLingo 账号</p>
      </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <button 
          type="submit" 
          :disabled="isLoading"
          class="w-full py-3 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50"
        >
          {{ isLoading ? '登录中...' : '登录' }}
        </button>
      </form>
      ...
      <div class="mt-6 text-center text-sm text-gray-500">
        还没有账号？ 
        <router-link to="/register" class="text-indigo-600 font-semibold hover:text-indigo-700">立即注册</router-link>
      </div>
    </div>
  </div>
</template>
