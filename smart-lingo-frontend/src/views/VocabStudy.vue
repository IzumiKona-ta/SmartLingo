...
const STORAGE_KEY = 'word-study-session'
...
const saveSession = () => {
  const session = {
    ...
    currentBookCode: currentBookCode.value,
    currentBookLabel: currentBookLabel.value,
    timestamp: Date.now()
  }
...
const loadSession = () => {
  ...
  try {
    ...
    // Strict Validation
    if (!session.words || !Array.isArray(session.words) || session.words.length === 0) return false
    
    // Check if words are valid objects
    const hasInvalidWords = session.words.some(w => !w || !w.word)
    if (hasInvalidWords) {
        console.warn("Invalid words in session, clearing.")
        localStorage.removeItem(STORAGE_KEY)
        return false
    }

    words.value = session.words
    currentIndex.value = typeof session.currentIndex === 'number' ? session.currentIndex : 0
    reviewedWords.value = Array.isArray(session.reviewedWords) ? session.reviewedWords : []
    masteredCount.value = typeof session.masteredCount === 'number' ? session.masteredCount : 0
    initialCount.value = typeof session.initialCount === 'number' ? session.initialCount : session.words.length
    
    // Only restore book info if it's valid (not Loading...)
    if (session.currentBookLabel && session.currentBookLabel !== 'Loading...') {
        currentBookCode.value = session.currentBookCode || 'CET4'
        currentBookLabel.value = session.currentBookLabel
    }
...
watch([currentIndex, masteredCount, words, currentBookLabel], () => {
    // Don't save if loading or if book label is invalid
    if (isLoading.value || currentBookLabel.value === 'Loading...') return
    saveSession()
}, { deep: true })
...
onMounted(async () => {
  ...
  if (loaded) {
      // Fetch only stats to keep UI updated
      try {
          const userId = getUserId()
          const userRes = await axios.get(`/api/user/stats?userId=${userId}`)
          if (userRes.data.stats) {
